        ...
        const run = () => {
            ...
            const tick = (now) => {
                const p = Math.min(1, (now - start) / duration),
                    e = 1 - Math.pow(1 - p, 3);
                setDisplay(Math.round(target * e).toLocaleString());
...
function Steps({ step }) {
    return (
        <div className="steps">
            {["Train", "Seats", "Passengers", "Payment", "Ticket"].map((x, i) => (
                <div className={i <= step ? "done" : ""} key={x}>
                    <span className={i === step ? "flip" : ""}>
...
function App() {
    ...
    const [user, setUser] = useState(() =>
        JSON.parse(localStorage.getItem("rail-user") || "null")
    );
    ...
    const doSearch = async () => {
        ...
        try {
            const trips = await api(
                `/trains/search?from=${encodeURIComponent(
                    search.from
                )}&to=${encodeURIComponent(search.to)}&date=${encodeURIComponent(
                    search.date
                )}`
            );
            const enriched = await Promise.all(
                trips.map(async (t) => {
                    ...
                    try {
                        classes = await api(
                            `/bookings/classes?tripId=${t.trip_id}&sourceStationId=${t.source_station_id}&destinationStationId=${t.destination_station_id}`
                        );
                    } catch {
                        classes = [];
                    }
    ...
    const chooseTrain = async (t, c) => {
        ...
        try {
            const data = await api(
                `/bookings/seats?tripId=${t.trip_id}&sourceStationId=${t.source_station_id}&destinationStationId=${t.destination_station_id}&classId=${c.classId}`
            );
    ...
    const estimatedTotal =
        (selectedClass?.farePerPassenger || 0) * search.passengers;
    ...
    const toPayment = async () => {
        if (
            passengers.some(
                (p) => !p.name || !p.age || Number(p.age) < 1 || Number(p.age) > 120
            )
        ) {
    ...
    const cancelBooking = async (pnr) => {
        ...
        setLoading(true);
        try {
            const result = await api(`/bookings/${pnr}/cancel`, {
                method: "POST",
            });
            setToast(
    ...
    return (
        <div id="root" className="app">
            ...
            <div key={page} className="page-transition">
                ...
                {page === "confirmation" && (
                    <Confirmation booking={lastBooking} navigate={navigate} />
                )}
                {page === "dashboard" && (
                    <Dashboard
                        user={user}
...
function Home({ search, setSearch, doSearch, navigate, stations }) {
    return (
        <>
            <section className="hero home-hero-clean">
                <div className="hero-copy">
                    <span className="eyebrow">BANGLADESH RAILWAY E-TICKETING</span>
                    <h1>
                        Travel smarter.
                        <br />
                    ...
                    <div className="hero-actions">
                        <button className="primary" onClick={doSearch}>
                            Book your journey <Icon name="arrow" size={18} />
                        </button>
                        <button className="ghost-button" onClick={() => navigate("track")}>
                            <Icon name="train" size={18} /> Track a train
                        </button>
                    </div>
                ...
                <div className="train-art hero-visual-card">
                    <div className="hero-orbit"></div>
                    <img
                        className="hero-train-image"
                        src={heroTrain}
                        alt="Modern intercity train illustration"
                    />
                </div>
            </section>
            ...
            <section className="feature-strip reveal">
                ...
                <article>
                    ...
                    <div>
                        <Counter value="60" suffix=" min" />
                        <span>delay threshold for spare rotation</span>
                    </div>
                </article>
                <article>
...
function SkeletonCard() {
    return (
        <div className="train-card card skeleton-card">
            <div className="sk-row sk-w40"></div>
            <div
                className="sk-row sk-w70"
                style={{ height: 34, marginTop: 18 }}
            ></div>
            <div
                className="sk-row sk-w100"
...
function SearchPage({
    ...
    const [times, setTimes] = useState([]),
        ...
        [sort, setSort] = useState("earliest");
    const filtered = useMemo(() => {
        let list = [...trains];
        if (times.length)
            list = list.filter((t) => {
                const h = new Date(t.scheduled_departure).getHours();
                return times.some((x) =>
                    x === "Morning"
                        ? h >= 5 && h < 12
                        : x === "Afternoon"
        ...
        if (types.length)
            list = list.filter((t) =>
                types.includes(String(t.train_type || "").toUpperCase())
            );
        return list.sort((a, b) =>
            sort === "lowest"
                ? Math.min(
                    ...(a.classes || []).map((c) =>
                        Number(c.farePerPassenger || 999999)
                    )
                ) -
                Math.min(
                    ...(b.classes || []).map((c) =>
            ...
            <div className="results-layout">
                <aside className="filters card">
                    ...
                    {["Morning", "Afternoon", "Evening", "Night"].map((x) => (
                        <label className="check" key={x}>
                            <input
                                type="checkbox"
                                checked={times.includes(x)}
                                onChange={() => toggle(setTimes, x)}
                            />
                            <span></span>
                            {x}
                        </label>
                    ...
                    {["INTERCITY", "EXPRESS", "MAIL"].map((x) => (
                        <label className="check" key={x}>
                            <input
                                type="checkbox"
                                checked={types.includes(x)}
                                onChange={() => toggle(setTypes, x)}
                            />
                            <span></span>
                            {x}
                        </label>
                ...
                <section className="train-results">
                    ...
                    {loading ? (
                        ...
                        <div className="empty card reveal visible">
                            <Icon name="search" size={34} />
                            <h2>No scheduled trip found</h2>
                            <p>
                                Try a different route/date, or create the dated trip from the
                                Admin panel.
                            </p>
                        </div>
                    ) : (
                        filtered.map((t, i) => (
                            <article
                                className="train-card card reveal"
                                ...
                                <div className="train-head">
                                    <div>
                                        ...
                                        <h3>
                                            {t.train_name}
                                            <small>
                                                {t.train_code} • {t.train_type} • {t.direction}
                                            </small>
                                        </h3>
                                    </div>
                                    <div className="train-tools">
                                        <button
                                            className={
                                                favorites.includes(t.train_id)
                                                    ? "favorite active"
                                                    : "favorite"
                                            }
                                            onClick={() => toggleFavorite(t.train_id)}
                                            title="Save train"
                                        >
                                            <Icon name="heart" size={19} />
                                        </button>
                                        <span
                                            className={`live-pill ${Number(t.current_delay_minutes) > 0 ? "late" : "ontime"
                                                }`}
                                        >
                                            {delayText(t.current_delay_minutes)}
                                        </span>
                                    </div>
                                </div>
                                <div className="timeline">
                                    <div>
                                        <b>{fmtTime(t.scheduled_departure)}</b>
                                        <span>{t.source_station}</span>
                                    </div>
                                    <div className="duration">
                                        <span>
                                            <Icon name="clock" size={14} />
                                            {durationText(t.scheduled_departure, t.scheduled_arrival)}
                                        </span>
                                        <i></i>
                                        <small>{t.trip_status}</small>
                                    </div>
                                    <div>
                                        <b>{fmtTime(t.scheduled_arrival)}</b>
                                        <span>{t.destination_station}</span>
                                    </div>
                                </div>
                                {t.last_left_station && (
                                    <div className="inline-live">
                                        <Icon name="info" size={15} /> Last left{" "}
                                        {t.last_left_station} at {fmtTime(t.last_left_at)} •{" "}
                                        {delayText(t.current_delay_minutes)}
                                    </div>
                                )}
                                <button
                                    className="details-link"
                                    onClick={() => setDetailTrain(t)}
                                >
                                    <Icon name="info" size={16} /> View trip details
                                </button>
                                <div className="classes">
                                    {(t.classes || []).length ? (
                                        (t.classes || []).map((c) => (
                                            <button
                                                key={c.classId}
                                                disabled={!c.availableSeats}
                                                onClick={() => chooseTrain(t, c)}
                                            >
                                                <span>
                                                    <b>{c.className}</b>
                                                    <small>{c.availableSeats} seats available</small>
                                                </span>
                                                <strong>{money(c.farePerPassenger)}</strong>
                                                <em>
                                        ...
                                        <div className="class-empty">
                                            No fare/seat inventory configured for this trip.
                                        </div>
                                    )}
                                </div>
                            </article>
...
function Summary({ train, cls, search, seatLabels, total }) {
    return (
        <aside className="summary card">
            ...
            <div className="summary-train">
                ...
                <div>
                    ...
                    <small>
                        {train?.train_code} • {cls?.className}
                    </small>
                </div>
            </div>
            <dl>
                ...
                <dt>Time</dt>
                <dd>
                    {fmtTime(train?.scheduled_departure)} –{" "}
                    {fmtTime(train?.scheduled_arrival)}
                </dd>
                <dt>Seats</dt>
                <dd>{seatLabels?.length ? seatLabels.join(", ") : "Not selected"}</dd>
            </dl>
            <div className="total">
...
function SeatPage({
    ...
    return (
        <main className="page">
            <Steps step={1} />
            <div className="page-title">
                <span className="eyebrow">SEGMENT-AWARE SEAT SELECTION</span>
                <h1>{train?.train_name}</h1>
                <p>
                    {cls?.className} • Choose {search.passengers} seat(s)
                </p>
            </div>
            <div className="booking-layout">
                <section className="seat-panel card">
                    <div className="coach-header">
                        <div>
                            <b>{cls?.className}</b>
                            <span>
                                Only non-overlapping seats for {search.from} → {search.to} are
                                selectable
                            </span>
                        </div>
                        <div className="legend">
                            ...
                            <span>
                                <i className="blocked"></i>Booked/Held
                            </span>
                        </div>
                    </div>
                    {!grouped.length ? (
                        <div className="empty">
                            <p>No seat inventory found.</p>
                        </div>
                    ) : (
                        grouped.map(([coach, list]) => (
                            <div className="coach-block" key={coach}>
                                <h4>Coach {coach}</h4>
                                <div className="coach">
                                    <div className="driver">
                                        <Icon name="train" size={26} />
                                    </div>
                                    <div className="seat-grid">
                                        {list.map((s, i) => {
                                            const id = Number(s.trip_seat_id),
                                                blocked = Number(s.is_available) !== 1;
                                            return (
                                                <button
                                                    aria-label={`Seat ${coach}-${s.seat_number}`}
                                                    key={id}
                                                    disabled={blocked}
                                                    className={
                                                        blocked
                                                ...
                                                >
                                                    {s.seat_number}
                                                    {i % 4 === 1 && <span className="aisle"></span>}
                                                </button>
                                            );
                                        })}
                                    </div>
                    ...
                    <div className="actions">
                        ...
                        <button className="primary" onClick={next}>
                            Continue to passengers <Icon name="arrow" size={17} />
                        </button>
                    </div>
                </section>
                <Summary
            ...
            <div className="booking-layout">
                <section className="passenger-panel card">
                    ...
                    <div className="actions">
                        ...
                        <button className="primary" onClick={next}>
                            Hold seats & continue <Icon name="arrow" size={17} />
                        </button>
                    </div>
                </section>
                <Summary
    ...
    if (!booking)
        return (
            <main className="page empty">
                <h2>No active seat hold</h2>
                <p>Return to seat selection and create a booking hold.</p>
            </main>
        );
    const expiry = booking.passengers?.find(
        (p) => p.hold_expires_at
    )?.hold_expires_at;
    return (
        <main className="page">
            ...
            <div className="booking-layout">
                <form className="payment card" onSubmit={(e) => confirm(e, method)}>
                    ...
                    <div className="pay-options">
                        {["Mobile Banking", "Card", "Bank Transfer"].map((x, i) => (
                            <label className={method === x ? "selected" : ""} key={x}>
                                <input
                                    type="radio"
                                    name="pay"
                                    checked={method === x}
                    ...
                    <div className="actions">
                        ...
                        <button className="primary">Pay {money(total)} & confirm</button>
                    </div>
                </form>
                <Summary
                    train={train}
                ...
                <div className="ticket">
                    <div className="ticket-main">
                        ...
                        <div className="ticket-route">
                            <div>
                                <b>{fmtTime(booking.scheduled_departure)}</b>
                                <span>{booking.source_station}</span>
                            </div>
                            ...
                            <div>
                                <b>{fmtTime(booking.scheduled_arrival)}</b>
                                <span>{booking.destination_station}</span>
                            </div>
                        </div>
                        <div className="ticket-info">
                            <span>
                                Date<b>{fmtDate(booking.scheduled_departure)}</b>
                            </span>
                            <span>
                                Class<b>{booking.class_name}</b>
                            </span>
                            <span>
                                Seat<b>{seats.join(", ")}</b>
                            </span>
                            <span>
                                Fare<b>{money(booking.total_fare)}</b>
                            </span>
                        </div>
                    </div>
                    <div className="qr">
                        ▦<small>PNR verified</small>
                    </div>
                </div>
...
function Dashboard({
    ...
    const confirmed = bookings.filter((b) => b.booking_status === "CONFIRMED"),
        spent = confirmed.reduce((sum, b) => sum + Number(b.total_fare || 0), 0);
    const upcoming = [...confirmed]
        .filter((b) => new Date(b.scheduled_departure) >= new Date())
        .sort(
            (a, b) =>
                new Date(a.scheduled_departure) - new Date(b.scheduled_departure)
        )[0];
        ...
        <main className="page dashboard-page">
            <section className="dashboard-hero">
                <div>
                    <span className="eyebrow">
                        {roleLabel(user.role).toUpperCase()} COMMAND CENTER
                    </span>
                    ...
                    <div className="hero-actions">
                        <button className="primary" onClick={() => navigate("search")}>
                            Plan a new journey <Icon name="arrow" size={18} />
                        </button>
            ...
            <section className="metric-grid">
                <article>
                    ...
                    <div>
                        <small>Confirmed journeys</small>
            ...
            <section className="dashboard-grid">
                <article className="card journey-focus">
                    <div className="section-head">
                        <div>
                            <span className="eyebrow">NEXT JOURNEY</span>
                            <h2>
                                {upcoming ? "Ready for departure" : "Nothing scheduled yet"}
                            </h2>
                        </div>
                        {upcoming && <span className="confirmed">CONFIRMED</span>}
                    </div>
                    {upcoming ? (
                        <>
                            <div className="journey-route">
                                <div>
                                    <b>{fmtTime(upcoming.scheduled_departure)}</b>
                                    <span>{upcoming.source_station}</span>
                                </div>
                                ...
                                <div>
                                    <b>{fmtTime(upcoming.scheduled_arrival)}</b>
                                    <span>{upcoming.destination_station}</span>
                                </div>
                            </div>
                            <div className="journey-meta">
                                <span>
                                    Date<b>{fmtDate(upcoming.scheduled_departure)}</b>
                                </span>
                                <span>
                                    Train<b>{upcoming.train_name}</b>
                                </span>
                                <span>
                                    Direction<b>{upcoming.direction}</b>
                                </span>
                                <span>
                                    PNR<b>{upcoming.pnr_number}</b>
                                </span>
                            </div>
                            <button className="text-btn" onClick={() => navigate("tickets")}>
                                View ticket wallet <Icon name="arrow" size={16} />
                            </button>
                        </>
                    ) : (
                ...
                <aside className="card quick-panel">
                    ...
                    <h2>Railway toolkit</h2>
                    {[
                        [
                            "train",
                            "Track live train",
                            "Last station and current delay",
                            "track",
                        ],
                        [
                            "ticket",
                        ...
                        ...(user.role === "ADMIN"
                            ? [
                                [
                                    "chart",
                                    "Admin operations",
                                    "Trips, trainsets and assignments",
                                    "admin-add-train",
                                ],
                            ]
                            : []),
                    ].map(([icon, title, copy, target]) => (
                        <button key={title} onClick={() => navigate(target)}>
                            <span>
...
function Tickets({ user, bookings, navigate, cancel, onAuth, handleError }) {
    ...
    const open = async (pnr) => {
        setDetailLoading(true);
        try {
            setSelected(await api(`/bookings/${pnr}`));
        } catch (err) {
            handleError(err);
        } finally {
    ...
    return (
        <main className="page">
            ...
            {detailLoading && <div className="mini-loading">Loading ticket…</div>}
            {!bookings.length ? (
                    ...
                    {bookings.map((b) => (
                        <article className="booking-card card" key={b.pnr_number}>
                            ...
                            <div className="booking-main">
                                ...
                                <div>
                                    <b>{fmtDate(b.scheduled_departure)}</b>
                                    <span>
                                        {fmtTime(b.scheduled_departure)} –{" "}
                                ...
                                <div>
                                    ...
                                    <span>{fmtDate(b.booking_time)}</span>
                                </div>
                            </div>
                            <div className="booking-actions">
                                <button onClick={() => open(b.pnr_number)}>
            ...
            {selected && (
                <BookingModal booking={selected} close={() => setSelected(null)} />
            )}
        </main>
    );
}
...
function TrackTrain({ handleError }) {
    ...
    const track = async (e) => {
        ...
        try {
            const q = query.trim();
            const data = /^\d+$/.test(q)
                ? await api(`/trips/${q}/status`)
                : await api(`/trains/${encodeURIComponent(q)}/status`);
            setLive(data);
    ...
    return (
        <main className="page">
            <div className="page-title">
                <span className="eyebrow">NO GPS REQUIRED</span>
                <h1>Track train operation</h1>
            ...
            {loading ? (
                ...
                live && (
                    <>
                        <section className="live-status-card card">
                            <div className="live-status-head">
                                <div>
                                    ...
                                    <div>
                                        <span className="eyebrow">
                                            TRIP #{live.trip_id} • {live.direction}
                                        </span>
                                        <h2>{live.train_name}</h2>
                                        <p>
                                            {live.train_code} • {live.trip_status}
                                        </p>
                                    </div>
                                </div>
                                <span
                                    className={`delay-badge ${Number(live.current_delay_minutes) > 0 ? "late" : "ontime"
                                        }`}
                                >
                                    {delayText(live.current_delay_minutes)}
                                </span>
                            </div>
                            <div className="live-facts">
                                <article>
                                    <small>Current operational position</small>
                                    <b>
                                        {atStop
                                            ? `At ${atStop.station_name}`
                                            : live.last_left_station
                                                ? `Left ${live.last_left_station}`
                                                : live.trip_status === "COMPLETED"
                                                    ? "Journey completed"
                                                    : "Not departed yet"}
                                    </b>
                                    <span>
                                        {atStop
                                            ? `Arrived ${fmtTime(atStop.actual_arrival)}`
                                            : live.last_left_at
                                ...
                                <article>
                                    ...
                                    <b>
                                        {atStop ? "Waiting to depart" : live.next_station || "—"}
                                    </b>
                                    <span>
                                        Scheduled{" "}
                                        {fmtTime(
                                            live.next_scheduled_arrival ||
                                            live.next_scheduled_departure
                                        )}
                                    </span>
                                </article>
                                <article>
                                    <small>Original schedule</small>
                                    <b>
                                        {fmtTime(live.scheduled_departure)} →{" "}
                                        {fmtTime(live.scheduled_arrival)}
                                    </b>
                                    <span>Timetable stays unchanged when delayed</span>
                                </article>
                                <article>
                                    <small>Spare rotation</small>
                                    <b>
                                        {Number(live.spare_triggered)
                                            ? "Triggered"
                                            : "Not triggered"}
                                    </b>
...
function StopTimeline({ stops }) {
    return (
        <section className="card stop-timeline">
            <div className="section-head">
                <div>
                    <span className="eyebrow">OPERATOR EVENT LOG</span>
                    <h2>Station progression</h2>
                </div>
            </div>
            {stops.map((s, i) => (
                <div
                    className={`stop-row ${String(s.stop_status).toLowerCase()}`}
                    key={s.trip_stop_id}
                >
                    <div className="stop-dot">
                        {s.actual_departure ||
                            (!s.scheduled_departure && s.actual_arrival) ? (
        ...
        <main className="page">
            ...
            {!notifications.length ? (
                ...
                <div className="notification-list">
                    {notifications.map((n) => (
                        <article
                            className={`card notification-card ${Number(n.is_read) ? "read" : "unread"
                                }`}
                        ...
                        >
                            ...
                            {!Number(n.is_read) && (
                                <button onClick={() => read(n.notification_id)}>
                                    Mark read
                                </button>
                            )}
                        </article>
...
function OperatorPanel({ user, handleError, setToast }) {
    ...
    const loadTrips = useCallback(async () => {
        ...
        try {
            ...
            setTrips(data);
            if (selected && !data.some((t) => Number(t.trip_id) === Number(selected)))
                setSelected(null);
        } catch (err) {
            handleError(err);
        } finally {
    ...
    const open = async (id) => {
        ...
        setLoading(true);
        try {
            setOps(await api(`/operator/trips/${id}`));
        } catch (err) {
            handleError(err);
        } finally {
            setLoading(false);
    ...
    const mark = async (stop, action) => {
        ...
        try {
            const result = await api(
                `/operator/trips/${selected}/stops/${stop.trip_stop_id}/${action}`,
                {
                    method: "POST",
                }
            );
            if (action === "depart" && result.spare?.triggered)
                setToast(
                    result.spare.trainsetCode
                        ? `Delay ${result.delayMinutes} min: spare ${result.spare.trainsetCode} reserved for next opposite trip`
                        : `Delay threshold triggered: ${result.spare.reason}`
                );
            else if (result.destinationReached)
                setToast(
                    `Journey completed: ${result.rotation?.action || "trainset rotation updated"
                    }`
                );
            else
                setToast(
                    `${stop.station_name} ${action === "arrive" ? "arrival" : "departure"
                    } marked at server time`
                );
    ...
    return (
        <main className="page">
            ...
            <div className="operator-toolbar card">
                ...
                <button className="secondary" onClick={loadTrips}>
                    Refresh
                </button>
            </div>
            <div className="operator-layout">
                <aside className="card operator-trip-list">
                    <h3>Assigned trips</h3>
                    {loading && !trips.length && <p>Loading…</p>}
                    {!trips.length && !loading && <p>No trips assigned for this date.</p>}
                    {trips.map((t) => (
                        <button
                            className={Number(selected) === Number(t.trip_id) ? "active" : ""}
                            key={t.trip_id}
                            onClick={() => open(t.trip_id)}
                        >
                            <div>
                                <b>
                                    {t.train_name} <small>#{t.trip_id}</small>
                                </b>
                                <span>
                            ...
                            <div>
                                <strong>{fmtTime(t.scheduled_departure)}</strong>
                                <small>
                                    {t.trip_status} • {delayText(t.current_delay_minutes)}
                                </small>
                            </div>
                        </button>
                    ))}
                </aside>
                <section className="card operator-stops">
                    {!ops ? (
                        <div className="empty">
                            <Icon name="clock" size={38} />
                            <h2>Select a trip</h2>
                            <p>Then mark Arrived / Departed in station order.</p>
                        </div>
                    ) : (
                        <>
                            <div className="section-head">
                                <div>
                                    <span className="eyebrow">
                                        TRIP #{ops.trip.trip_id} • {ops.trip.direction}
                                    </span>
                                    <h2>{ops.live?.train_name}</h2>
                                    <p>
                                        {ops.live?.last_left_station
                                            ? `Last left ${ops.live.last_left_station} at ${fmtTime(
                                                ops.live.last_left_at
                                            )}`
                                            : "Awaiting first departure"}{" "}
                                        • {delayText(ops.live?.current_delay_minutes)}
                                    </p>
                                </div>
                                <span
                                    className={`delay-badge ${Number(ops.live?.current_delay_minutes) > 0
                                        ? "late"
                                        : "ontime"
                                        }`}
                                >
                                    {ops.trip.trip_status}
                                </span>
                            </div>
                            <div className="operator-stop-list">
                                {ops.stops.map((s, i) => {
                                    const departureLocked =
                                        s.scheduled_departure &&
                                        now < new Date(s.scheduled_departure).getTime();

                                    return (
                                        <article className="operator-stop" key={s.trip_stop_id}>
                                            <div className="stop-dot">
                                                {s.actual_departure ||
                                                    (!s.scheduled_departure && s.actual_arrival) ? (
                                                    <Icon name="check" size={14} />
                                            ...
                                            <div className="operator-stop-main">
                                                <b>{s.station_name}</b>

                                                <span>
                                                    Scheduled{" "}
                                                    {s.scheduled_arrival &&
                                                        `Arr ${fmtTime(s.scheduled_arrival)}`}{" "}
                                                    {s.scheduled_departure &&
                                                        `Dep ${fmtTime(s.scheduled_departure)}`}
                                                </span>

                                                <small>
                                                    {s.actual_arrival &&
                                                        `Actual arrival ${fmtTime(s.actual_arrival)}`}{" "}
                                                    {s.actual_departure &&
                                                        `• Actual departure ${fmtTime(s.actual_departure)}`}
                                                </small>
                                            </div>
                                                ...
                                                {s.scheduled_departure &&
                                                    ...
                                                    (!s.scheduled_arrival || s.actual_arrival) && (
                                                        <button
                                                            ...
                                                            title={
                                                                departureLocked
                                                                    ? `Departure available at ${fmtTime(
                                                                        s.scheduled_departure
                                                                    )}`
                                                                    : "Mark train as departed"
                                                            }
                                                            onClick={() => mark(s, "depart")}
                                                        >
                                                            Departed
                                                        </button>
                                                    )}
...
function Support({ setToast }) {
    const [query, setQuery] = useState("");
    const faq = [
        "How do I book a ticket?",
        "How does live train tracking work?",
        "When is a spare train assigned?",
    ...
    return (
        <main className="page">
            <div className="page-title">
                ...
                <p>Find quick answers or send us a message.</p>
            </div>
                    ...
                    {faq
                        ...
                        .map((q) => (
                            <details key={q}>
                                ...
                                <p>
                                    {q.includes("tracking")
                                        ? "An operator marks Arrived/Departed. The server timestamp becomes the actual event time, and users see the last station left plus current delay without GPS."
                                        : q.includes("spare")
...
function TrainDetail({ train, close, choose }) {
    return (
        <div
            className="modal-backdrop"
            onMouseDown={(e) => e.target === e.currentTarget && close()}
            ...
            <div className="detail-modal card">
                ...
                <div className="detail-hero">
                    ...
                    <div>
                        ...
                        <p>
                            {train.train_code} • {train.train_type} • {train.direction}
                        </p>
                    </div>
                    <span
                        className={`live-pill ${Number(train.current_delay_minutes) > 0 ? "late" : "ontime"
                            }`}
                    >
                        {delayText(train.current_delay_minutes)}
                    </span>
                </div>
                <div className="detail-route">
                    <div>
                        <b>{fmtTime(train.scheduled_departure)}</b>
                    ...
                    <div>
                        ...
                        <Icon name="train" size={20} />
                        <small>
                            {durationText(train.scheduled_departure, train.scheduled_arrival)}
                        </small>
                        <i></i>
                    </div>
                    <div>
                        <b>{fmtTime(train.scheduled_arrival)}</b>
                        <span>{train.destination_station}</span>
                    </div>
                </div>
                {train.last_left_station && (
                    <p className="detail-live-copy">
                        Last station left: <b>{train.last_left_station}</b> at{" "}
                        {fmtTime(train.last_left_at)}.
                    </p>
                )}
    ...
    return (
        ...
        >
            <div className="detail-modal card">
                ...
                <span className="eyebrow">PNR {booking.pnr_number}</span>
                <h2>{booking.train_name}</h2>
                <div className="booking-detail-grid">
                    ...
                    <span>
                        Departure
                        <b>
                            {fmtDate(booking.scheduled_departure)}{" "}
                            {fmtTime(booking.scheduled_departure)}
                        </b>
                    </span>
                    <span>
                        Class<b>{booking.class_name}</b>
                    ...
                    <span>
                        Status<b>{booking.booking_status}</b>
                    </span>
                    <span>
                        Passengers<b>{people.map((p) => p.passenger_name).join(", ")}</b>
                    </span>
                    <span>
                        Total<b>{money(booking.total_fare)}</b>
                    </span>
                </div>
                    ...
                    {people.map((p) => (
                        <div key={p.passenger_id}>
                            ...
                            <span>
                                Seat{" "}
                                <b>
                                    {p.coach_code}-{p.seat_number}
                                </b>
                            </span>
                            <span>
                                Ticket <b>{p.ticket_status || "—"}</b>
                                {p.refund_status && (
                                    <small>
                                        Refund: {p.refund_status} ({money(p.refund_amount)})
                                    </small>
                                )}
                            </span>
                        </div>
                ...
                <button className="primary full" onClick={() => window.print()}>
                    <Icon name="print" size={17} /> Print ticket
                </button>
            </div>
        </div>
    );
...
function AccessCard({ title, copy, action, onAction }) {
    return (
        <main className="page">
