export async function getLiveTrip(connection, tripId) {
  const result = await connection.query(
    `SELECT * FROM VW_LIVE_TRAIN_STATUS WHERE TRIP_ID = $1`,
    [tripId]
  )
...
export async function getTrip(connection, tripId, forUpdate = false) {
  const result = await connection.query(
    `SELECT T.*, R.DIRECTION, R.SOURCE_STATION_ID, R.DESTINATION_STATION_ID,
      ...
      WHERE T.TRIP_ID = $1${forUpdate ? ' FOR UPDATE' : ''}`,
    [tripId]
  )
  return result.rows[0] || null
}
...
export async function markTripStarted(connection, tripId) {
  await connection.query(
    `UPDATE TRIPS
        SET ACTUAL_DEPARTURE = COALESCE(ACTUAL_DEPARTURE, CURRENT_TIMESTAMP),
            TRIP_STATUS = 'RUNNING'
      WHERE TRIP_ID = $1`,
    [tripId]
  )
...
export async function markSpareTriggered(connection, tripId) {
  await connection.query(
    `UPDATE TRIPS
        SET SPARE_TRIGGERED_AT = COALESCE(SPARE_TRIGGERED_AT, CURRENT_TIMESTAMP)
      WHERE TRIP_ID = $1`,
    [tripId]
  )
}
...
export async function findNextOppositeTrip(connection, trip) {
  const result = await connection.query(
    `SELECT NEXT_T.TRIP_ID, NEXT_T.TRAIN_ID, NEXT_T.ROUTE_ID, NEXT_T.SCHEDULED_DEPARTURE,
      ...
      WHERE NEXT_T.TRAIN_ID = $1
        AND NEXT_R.DIRECTION <> $2
        AND NEXT_R.SOURCE_STATION_ID = $3
        AND NEXT_T.SCHEDULED_DEPARTURE > $4
        AND NEXT_T.TRIP_STATUS IN ('SCHEDULED','BOARDING')
      ORDER BY NEXT_T.SCHEDULED_DEPARTURE
      LIMIT 1`,
    [trip.TRAIN_ID, trip.DIRECTION, trip.DESTINATION_STATION_ID, trip.SCHEDULED_DEPARTURE]
  )
  return result.rows[0] || null
}

export async function listOperatorTrips(connection, operatorId, date) {
  const result = await connection.query(
    `SELECT T.TRIP_ID, TR.TRAIN_NAME, TR.TRAIN_CODE, R.DIRECTION,
      ...
      WHERE T.OPERATOR_USER_ID = $1
        AND DATE(T.JOURNEY_DATE) = $2::date
